...
const int Gbit = 64;   // may be 32/64/128; must match Gbit in common/graph.h
...
int check(int u, int v){
        ...
        if (find(Graph[u].begin(), Graph[u].end(), v) != Graph[u].end())
                ...
                ans2 = 0;
        // 0: GT did not exclude (no common bit) + true non-edge -> FP (GT misses it)
        // 1: GT did not exclude + true edge -> TN
        // 2: GT excluded (common bit) + true non-edge -> TP (correct exclusion)
        // 3: GT excluded + true edge -> violates losslessness, must not happen
        if (ans1 == 0 && ans2 == 0) return 0;
        if (ans1 == 0 && ans2 == 1) return 1;
        if (ans1 == 1 && ans2 == 0) return 2;
        return 3;
}
...
// Usage: argv[1]=1 sample-check only; argv[1]=2 build the MIS-bucket index and verify

int main(int argc, char** argv) {
    if (argc < 4) {
        cerr << "Usage: " << argv[0] << " <mode> <input> <output>\n"
             << "  mode 1 = sample-check only; mode 2 = build the MIS-bucket index and verify\n";
        return 1;
    }
    if (atoi(argv[1]) == 1) {
